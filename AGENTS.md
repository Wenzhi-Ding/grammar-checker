...
- **Version + changelog on every behavior change.** Any commit that changes user-visible behavior, prompts, or provider logic MUST bump `version` in `package.json` (`npm version patch|minor|major --no-git-tag-version`, which also syncs `package-lock.json`) and add a dated section to `CHANGELOG.md` describing what changed and why, in the same commit (or the release commit immediately following). Docs-only changes (AGENTS.md, specs, comments) do not require a bump.
